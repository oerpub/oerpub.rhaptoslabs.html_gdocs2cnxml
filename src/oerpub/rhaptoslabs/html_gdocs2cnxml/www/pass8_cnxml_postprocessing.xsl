...
<xsl:template match="cnx:link/@bookmark" mode="pass8"/>
		...
		<xsl:when test="string-length(@bookmark) &gt; 1">
		  ...
		  <xsl:choose>
		    <xsl:when test="$precedingID or $followingID">
		        ...
		        <xsl:apply-templates select="@*" mode="pass8"/>
...
<xsl:template match="cnxtra:bookmark" mode="pass8"/>

<!-- remove underline placeholder -->
<!--
<xsl:template match="cnx:underline" mode="pass8"/>
-->
